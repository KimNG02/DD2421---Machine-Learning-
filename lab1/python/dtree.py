...
def entropy(dataset):
    ...
    n = len(dataset) #how many samples
    nPos = len([x for x in dataset if x.positive]) #, count of positives, list of only the postive ones
    nNeg = n - nPos #count of negatives 
    if nPos == 0 or nNeg == 0: #if all are pos or neg
        return 0.0
    #entropy formula 
    #p = nPos/n
...
#how good is an attribute split
def averageGain(dataset, attribute):
    "Calculate the expected information gain when an attribute becomes known"
    weighted = 0.0
    for v in attribute.values: #loop over each possible value of the attribute
        subset = select(dataset, attribute, v) #all samples where attribute == v (one pile)
        weighted += entropy(subset) * len(subset) #compute entropy for each pile, len is how many are in that pile
    return entropy(dataset) - weighted/len(dataset) #entropy before - entropy after
...
#filter the dataset, return only samples where that attribute has a certain value
